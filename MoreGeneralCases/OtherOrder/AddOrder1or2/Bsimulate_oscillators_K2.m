function [z1,z2] = Bsimulate_oscillators_K2(N, omega, t_max, dt, K, K1)
    % 参数设定
    n = 3;  % 非线性项指数
...
%     %%%%%%%Asymmetric%%%%%%%%%%%%%%
%     unite1 = [0,0,0,0,2*pi/n,0,0,0,0,0,0,0,0,0,2*pi/n,0,0,0,0,0];
%     theta1 = repmat(unite1, 1, N/40-1)';
%     unite2 = [0,0,0,0,0,2*pi/n,0,0,0,0,0,0,0,0,0,2*pi/n,0,0,0,0];
%     theta2 = repmat(unite2, 1, N/40+1)';
    ...
    function dtheta = theta_dot(theta, omega, K, N, n)
        ...
        R = abs(z);  % R是z的模长
        phase = angle(z);  % 集体相位
        dtheta = omega - (K * R^n) * sin(n * (theta - phase)) - (K1 * R^2) * sin(2 * (theta - phase));  % 相位变化率
    end
