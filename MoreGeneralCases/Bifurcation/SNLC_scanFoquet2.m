...
% Poincaré & Floquet 参数
section_dir = -1;   % 仅用 Theta31 的过零，上穿(+1)/下穿(-1)
M_settle     = 6;
tol_section  = 1e-3;
perturb_var  = 'R31';
eps_perturb  = 1e-1;
...
title('Floquet multiplier vs. K (parallel, section on \Theta_{31})');
grid on;
...
function [ok, mu] = run_one_K(K, omega0, Delta, dt, Tmax, ...
    ...
    Nmax = round(Tmax/dt);
    phi_prev = wrapToPi(Theta31); % ★ 截面只看 Theta31
    t = 0;
    ...
    for k = 1:Nmax
        ...
        % ★ 仅用 Theta31 判定切面穿越，并保留 cos(phi/3)>0 过滤
...
% ===== 下面三个函数与原脚本一致，但“切面判定”改为只看 Theta31 =====
